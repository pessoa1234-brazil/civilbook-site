...
async function renderRdo(param) {
  ...
  app.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      ...
      <button class="btn" onclick="cbAssessor('abrirCom','rdo')" title="Abrir o assessor já sabendo que você está no RDO (f40)"><i class="ti ti-sparkles" aria-hidden="true"></i> Perguntar à IA</button>
    </div>
...
async function rdoCarregarThumbs(r) {
  ...
  for (let i = 0; i < r.fotos.length; i++) {
    ...
    if (!f.path) continue;
    try {
      // download-inline: esta URL vira o `src` de um <img> na própria tela (miniatura da foto do RDO). Com
      // `{ download }` o Storage manda attachment e o navegador baixa o arquivo em vez de desenhar a imagem.
...
function perguntarIARdo() {
  ...
  // a52 P2: o Assessor chega quando a aba dele abre — cbAssessor (js/app.js) o busca antes de chamar (e é ele
  // que avisa se não vier; o typeof aqui dava falso e o botão não fazia nada).
  cbAssessor("consultarProjeto", "Diário da obra", null, ctx.slice(0, 30000));
}
