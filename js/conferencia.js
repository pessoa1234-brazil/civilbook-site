...
const CONF = {
  ...
  _loaded: false,
  _compartilhados: null,   // ids que outro dono compartilhou COMIGO (a59; o Realtime só aceita estes)

  // ---------- Carga inicial (banco quando logado; senão localStorage) ----------
  // a59 (22/set/2026): a leitura passa pelo cbLerMeusRecursos (casca, js/app.js) — dono = eu MAIS os ids
  // compartilhados comigo. O select("*") sem filtro confiava na política projetos_select (0010), que tem
  // `or public.is_admin()`: a Conferência do fundador listava os projetos de TODAS as contas, e o _fromRow
  // os marcava como "compartilhado comigo" (dono ≠ eu). O link por token (?share=) não passa por aqui.
  async load() {
    const locais = CBStore.lsGet(this.KEY, []);
    if (CBStore.online()) {
      try {
        const { data, error, parcial, compartilhados } =
          await cbLerMeusRecursos("projetos", "projeto", { ordem: "created_at" });
        if (error) throw error;
        this._compartilhados = compartilhados;
        if (parcial && typeof toast === "function") toast("Projetos compartilhados com você podem não ter carregado.", "warn");
        let remoto = (data || []).map(this._fromRow);
        // Migração única: se o banco está vazio e há projetos locais antigos, sobe-os.
        // `parcial` NÃO conta como banco vazio (achado da revisão de 22/set/2026): quem só participa de
        // projetos compartilhados reenviaria o espelho como projetos NOVOS, dele, na 1ª falha de rede.
        if (!parcial && !remoto.length && locais.length) {
          remoto = [];
        ...
        this._cache = remoto;
        // Carga PARCIAL não reescreve o espelho (mesma revisão): ele é o que abre offline, e gravá-lo sem
        // os compartilhados os apagava do aparelho — o toast avisa nesta sessão, mas não volta na próxima.
        if (!parcial) CBStore.lsSet(this.KEY, this._cache);
        this._loaded = true;
        return;
  ...
  escutarRealtime() {
    ...
    this._rtCh = SHARE.assinar("projetos", (payload) => {
      try {
        ...
        // a59: o Realtime aplica a MESMA política projetos_select, com o `or is_admin()` — sem esta guarda,
        // o projeto de outra conta entrava no cache do admin ao vivo, depois de a carga inicial já filtrar.
        // O DELETE fica FORA da régua de dono: o SDK monta SEMPRE `new: {}` (js/vendor/supabase.js,
        // _getPayloadRecords), então um `if (novo && …)` engolia TODO delete — o projeto apagado no celular
        // ficava na tela do notebook, editável, e o update por id não casava linha nenhuma (que o PostgREST
        // devolve como sucesso: b10/c9). Quem decide o DELETE é o cache: id que eu não tenho é no-op.
        const ehDelete = payload.eventType === "DELETE";
        if (!ehDelete && !cbRecursoEhMeu(novo, this._compartilhados)) return;
        if (ehDelete) {
          if (!antigo || !antigo.id || !this.projetos().some(p => p.id === antigo.id)) return;
...
const CONFRES = {
  ...
  // a59 (22/set/2026), ACHADO no inventário: a conferencia_resultados (0008) tem conf_res_own_rw (dono) E
  // conf_res_admin_read (select para o admin) — duas políticas somam. A leitura sem filtro entregava ao
  // fundador o histórico de auditoria de todas as contas (nome do projeto, RT, CREA, não conformidades).
  // Tabela sem compartilhamento: aqui o filtro é o uid puro, e a tabela vai por nome, não por this.TABLE
  // (nome em variável escapa da varredura de leitura sem filtro).
  async historico(projetoId) {
    const uid = CBStore.uid();
    if (CBStore.online() && uid) {
      try {
        let q = window.supa.from("conferencia_resultados").select("*").eq("user_id", uid)
