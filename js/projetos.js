...
const PROJ = cbColecao("projetos", "cb-projetos", rowToProj, projToRow);
...
async function gerarManualObra(id) {
  ...
  try {
    ...
    diz(`Pronto: ${d.nome} — ${(d.sistemas || []).length} sistema(s) condicionais detectados, ${d.resumo && d.resumo.fornecedores || 0} fornecedor(es). Baixando…`);
    if (d.url) window.open(d.url, "_blank", "noopener");
    if (typeof pdocReady === "function") { /* o doc novo aparece no acervo no próximo render */ }
  } catch (e) {
    diz("Erro ao gerar: " + (e && e.message ? e.message : e));
  }
}
...
function renderProjetoDetalhe(id) {
  ...
  const tipoIcone = { foto: "ti-photo", planta: "ti-blueprint", memorial: "ti-file-text", documento: "ti-file", outro: "ti-file" };
...
async function baixarProjDoc(docId) {
  ...
  try {
    const { data, error } = await window.supa.storage.from(PROJ_BUCKET).createSignedUrl(d.arquivo_path, 3600);
    if (error) throw error;
    window.open(data.signedUrl, "_blank", "noopener");
  } catch (e) { toast("Não foi possível abrir o arquivo.", "warn"); }
}
...
function perguntarIAProjeto(projetoId) {
  ...
  if (typeof ASSIST !== "undefined" && ASSIST.consultarProjeto) ASSIST.consultarProjeto(p ? p.nome : "Projeto", projetoId, ctx);
  else navigate("assessor");
}
