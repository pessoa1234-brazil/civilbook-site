...
(async () => {
  ...
  if (cfg.SUPA_READY) {
    try {
      ...
      marcar("cb:sdk-pronto");   // a47: entra no diário do boot (js/auth.js, cb-boot-log)
    } catch (e) {
      console.error("Civilbook: falha ao carregar o SDK Supabase —", e);
      window.supa = null;
      marcar("cb:sdk-falhou");
    }
  } else {
    window.supa = null;
    // a50 (19/set/2026): sem o js/config.js (bloqueado no DevTools, 404, rede) NÃO é modo local — o AUTH trata como
    // produção sem login (boot sem sessão, "a configuração do app não carregou" nos Detalhes). Dizer "modo local"
    // aqui contradizia a tela e mandava quem estivesse depurando para o lado errado (céticos da a50).
    if (!window.CB_CONFIG) console.error("Civilbook: a configuração do app (js/config.js) não carregou — login indisponível.");
    else console.info("Civilbook: Supabase não configurado — modo local (localStorage).");
  }
  ...
  // a47 (19/set/2026): o mesmo com a sessão GUARDADA no SDK, mesmo sem cache (prazo curto do getSession).
  try {
