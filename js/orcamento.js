...
function orcListaHTML() {
  ...
  return `
    ...
    <div style="margin:12px 0;display:flex;gap:10px;flex-wrap:wrap"><button class="btn primary" onclick="orcNovo()"><i class="ti ti-plus"></i> Novo orçamento</button>${typeof EXEMPLO !== "undefined" ? EXEMPLO.botaoBarraHTML("orcamento") : ""}<button class="btn" onclick="typeof ASSIST!=='undefined'&&ASSIST.abrirCom('orcamento')" title="Abrir o assessor já sabendo que você está no Orçamento (f40)"><i class="ti ti-sparkles" aria-hidden="true"></i> Perguntar à IA</button></div>
...
function orcGerarCronograma(id) {
