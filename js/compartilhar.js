...
async function renderSharedToken(token) {
  ...
  // a52 P2 (21/set/2026): a vista por link de um PROJETO de conferência usa CONF (js/conferencia.js) e o catálogo
  // CONFERENCIA (data/conferencia.js), que deixaram de vir na abertura. Esta vista roda SEM sessão e fora do
  // navigate(), então é ela que pede o módulo. Se não vier, cai no ramo de baixo (ordem de serviço), que com um
  // recurso de projeto mostraria campos vazios — por isso a falha vira a mesma tela de "Link indisponível" com
  // o motivo, nunca uma tela meio desenhada.
  const naoAbriu = (frase) => `<div class="card" style="max-width:520px;margin:48px auto;text-align:center;padding:32px">
        ...
        <p style="color:var(--text-2)">Esta conferência ${frase}</p>
        <button type="button" class="btn primary" style="margin-top:12px" onclick="location.reload()"><i class="ti ti-refresh"></i> Tentar de novo</button></div>`;
  if (r.tipo === "projeto" && typeof MODULOS !== "undefined" && MODULOS.faltam("conferencia").length) {
    try { await MODULOS.garantir("conferencia"); }
    catch (e) { app.innerHTML = naoAbriu(MODULOS.motivoTexto((e && e.motivo) || "rede")); return; }
  }
  // E se o próprio REGISTRO não chegou (o js/modulos.js é uma tag do app.html, e na 1ª abertura depois de
  // publicar o service worker não tem cópia dele), não há como buscar nada: sem esta saída, o `typeof CONF`
  // abaixo dá falso e o link de uma CONFERÊNCIA cai no ramo da ordem de serviço, desenhando um cartão com
  // "Ordem de serviço" e status "—". É a tela meio desenhada que o comentário acima diz impedir.
  if (r.tipo === "projeto" && (typeof CONF === "undefined" || typeof CONFERENCIA === "undefined")) {
    app.innerHTML = naoAbriu("não chegou. Verifique a conexão e tente de novo.");
    return;
  }
