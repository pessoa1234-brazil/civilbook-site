...
if (typeof document !== "undefined") {
  ...
  // a51 (19/set/2026): os modelos (data/param-pam.js, ~231 KB, e data/param-escola.js, ~288 KB) chegam SOB DEMANDA,
  // só quando o Paramétrico abre para quem é PRO. Antes eram <script> síncronos no app.html: toda abertura do app
  // baixava e interpretava meio megabyte que só esta sub-aba usa. Mesmo padrão do ensureSinapiData (js/sinapi.js).
  // A ORDEM IMPORTA: um arquivo por vez, na ordem da lista. O registry PARAM_MODELOS guarda as tipologias na ordem em
  // que os arquivos rodam, e o modelo padrão é o primeiro (saude, do param-pam). Baixar os dois juntos deixaria rodar
  // primeiro o que chegasse primeiro. Arquivo que já rodou (um app.html antigo do cache, com os <script> síncronos) não
  // é baixado de novo; arquivo que chega mas não define o modelo (resposta errada) conta como falha e para a fila.
  // Falha: a promessa zera e o próximo pedido (o "Tentar de novo") retoma do arquivo que faltou. O QUE JÁ CARREGOU
  // FICA USÁVEL — a fila para no 1º que falha, então quem chegou são as primeiras tipologias da lista (a padrão
  // inclusive): o formulário abre com elas e a falta vira aviso. Só a falha do 1º arquivo esvazia a sub-aba.
  // Tipologia nova = um item novo NESTA lista, nunca um <script> no app.html. Travado por tests/abertura.check.mjs.
  const PARAM_ARQUIVOS = [
    ["data/param-pam.js?v=cd8b896", function () { return typeof PARAM_PAM !== "undefined" && !!PARAM_PAM; }],
    ["data/param-escola.js?v=cd8b896", function () { return typeof PARAM_MODELOS !== "undefined" && !!PARAM_MODELOS && !!PARAM_MODELOS.escola; }]
  ];
  let _parDadosP = null;
  // Motivo da última falha, e nada quando tudo carregou: "rede" (o arquivo não chegou) ou "conteudo" (chegou e não
  // trouxe o modelo — cópia guardada com defeito, proxy, portal de wi-fi). O texto muda: mandar "verifique a conexão"
  // a quem está com a conexão boa põe a pessoa num laço, tentando de novo o que vai falhar igual (correção 19/set/2026).
  let _parIncompleto = null;
  function parErro(motivo, src) { const e = new Error(motivo + ": " + src); e.motivo = motivo; return e; }
  function parBaixarArquivo(src) {
    return new Promise(function (resolve, reject) {
      const s = document.createElement("script");
      s.src = src;
      s.onload = function () { resolve(); };
      s.onerror = function () { try { s.remove(); } catch (e) {} reject(parErro("rede", src)); };
      document.head.appendChild(s);
    });
  }
  function ensureParamData() {
    if (PARAM_ARQUIVOS.every(function (a) { return a[1](); })) return Promise.resolve();
    if (_parDadosP) return _parDadosP;
    _parDadosP = PARAM_ARQUIVOS.reduce(function (fila, a) {
      return fila.then(function () {
        if (a[1]()) return null;
        return parBaixarArquivo(a[0]).then(function () { if (!a[1]()) throw parErro("conteudo", a[0]); });
      });
    }, Promise.resolve()).catch(function (e) { _parDadosP = null; throw e; });
    return _parDadosP;
  }
  // A ponte do app.html (script cb-ponte-parametrico) reconhece o js novo por ESTE nome: renomear = mudar lá também (R8).
  window.ensureParamData = ensureParamData;

  // Uma frase por motivo, usada na tela de falha e no aviso do formulário incompleto.
  function parMotivoTexto(motivo) {
    return motivo === "conteudo"
      ? "chegaram incompletos — a cópia guardada no aparelho tem defeito. Feche e abra o app para buscar uma cópia nova."
      : "não chegaram. Verifique a conexão e tente de novo.";
  }
  // NENHUMA tipologia carregou: a sub-aba inteira vira a mensagem.
  function parFalhaHTML(motivo) {
    return `<div class="card" style="max-width:560px;margin:30px auto;text-align:center;padding:28px">
      <p style="color:var(--text-2);margin:0 0 16px">Os modelos do orçamento paramétrico ${parMotivoTexto(motivo)}</p>
      <button type="button" class="btn primary" onclick="renderParametrico()"><i class="ti ti-refresh"></i> Tentar de novo</button>
    </div>`;
  }
  // ALGUMA tipologia carregou (ex.: a saúde chegou e a escola não): o formulário abre com o que existe e a falta vira
  // um aviso com "Tentar de novo" no topo — apagar a sub-aba inteira tiraria da pessoa o que já está no aparelho.
  function parAvisoFaltaHTML(motivo) {
    return `<div class="card" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:14px">
      <span style="flex:1;min-width:240px;font-size:13px"><i class="ti ti-alert-triangle" style="color:#c97a00"></i> Algumas tipologias ${parMotivoTexto(motivo)} As que abriram funcionam normalmente.</span>
      <button type="button" class="btn" onclick="renderParametrico()"><i class="ti ti-refresh"></i> Tentar de novo</button>
    </div>`;
  }
  ...
  async function renderParametrico(pane) {
    ...
    const alvo = _parPane;
    if (!alvo) return;
    if (typeof planoEhPro === "function" && !planoEhPro()) { alvo.innerHTML = parUpsellHTML(); return; }
    alvo.innerHTML = `<p class="page-sub">Carregando…</p>`;
    // trocou de aba (ou abriu de novo) durante o await: o painel não é mais este
    const saiu = function () { return _parPane !== alvo || !document.body.contains(alvo); };
    // a base SINAPI (lazy ~1,7 MB; SINAPIDB.meta()/lista() dependem dela) baixa JUNTO com os modelos
    const sinapi = typeof SINAPIDB !== "undefined" ? Promise.resolve().then(function () { return SINAPIDB.ready(); }).catch(function () {}) : null;
    let falhou = null;
    try { await ensureParamData(); }
    catch (e) { falhou = (e && e.motivo) || "rede"; }
    if (saiu()) return;
    // O que CARREGOU fica usável: a fila para no 1º arquivo que falha, então o que já rodou são as PRIMEIRAS
    // tipologias da lista (a padrão inclusive) e o formulário abre com elas, com o aviso do que faltou. Só quando
    // NENHUMA chegou a sub-aba vira a tela de falha.
    _parIncompleto = falhou;
    if (!parModelo()) { alvo.innerHTML = falhou ? parFalhaHTML(falhou) : `<p class="page-sub">Modelo paramétrico indisponível.</p>`; return; }
    if (sinapi) await sinapi;
    if (saiu()) return;
    alvo.innerHTML = parFormHTML();
    parCalc();
  }
  ...
  function parFormHTML() {
    ...
    return `
      ${_parIncompleto ? parAvisoFaltaHTML(_parIncompleto) : ""}
