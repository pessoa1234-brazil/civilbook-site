...
const MNT = {
  ...
  _loaded: false,
  _compartilhados: null,   // ids de OS que outro dono compartilhou COMIGO (a59)

  // ---------- Carga inicial (banco quando logado; senão localStorage) ----------
  // a59 (22/set/2026): as OSs passam pelo cbLerMeusRecursos (casca, js/app.js) — dono = eu MAIS os ids
  // compartilhados comigo. O select("*") sem filtro confiava na política os_select (0010), que tem
  // `or public.is_admin()`: a Manutenção do fundador listava as OSs de TODAS as contas, com o selo de
  // compartilhado (o _osFromRow marca dono ≠ eu). A manutencao_execucoes é só do dono (0001, sem admin),
  // e o filtro explícito fica como o resto: a tela não depende do RLS para saber de quem é o dado.
  async load() {
    ...
    if (CBStore.online()) {
      try {
        const [ro, re] = await Promise.all([
          cbLerMeusRecursos("ordens_servico", "os", { ordem: "num" }),
          window.supa.from("manutencao_execucoes").select("*").eq("user_id", CBStore.uid())
        ]);
        if (ro.error) throw ro.error;
        if (re.error) throw re.error;
        this._compartilhados = ro.compartilhados;
        if (ro.parcial && typeof toast === "function") toast("Ordens de serviço compartilhadas com você podem não ter carregado.", "warn");
        ...
        // `ro.parcial` NÃO conta como banco vazio (revisão de 22/set/2026): com a lista de convites fora do
        // ar, quem só participa de OSs compartilhadas reenviaria o espelho como OS nova, dela.
        if (!ro.parcial && !os.length && osLocal.length) {
          os = [];
        ...
        // Carga PARCIAL não reescreve o espelho das OSs — sem os compartilhados ele as apaga do aparelho,
        // e offline o aviso não volta. O espelho das execuções é de outra consulta e segue gravando.
        if (!ro.parcial) CBStore.lsSet(this.KEY_OS, os);
  ...
  escutarRealtime() {
    ...
    this._rtCh = SHARE.assinar("ordens_servico", (payload) => {
      try {
        ...
        // a59: o Realtime aplica a mesma política os_select, com o `or is_admin()` — sem esta guarda, a OS
        // de outra conta entrava ao vivo no cache do admin. O DELETE fica FORA da régua de dono: o SDK
        // entrega `new: {}` nele (js/vendor/supabase.js), e um `if (novo && …)` engolia todo delete —
        // a OS apagada noutro aparelho ficava na tela. Quem decide o DELETE é o cache.
        const ehDelete = payload.eventType === "DELETE";
        if (!ehDelete && !cbRecursoEhMeu(novo, this._compartilhados)) return;
        if (ehDelete) {
          if (!antigo || !antigo.id || !this.listarOS().some(o => o.id === antigo.id)) return;
          this._os = this.listarOS().filter(o => o.id !== antigo.id);
...
// (planoEhPro MUDOU DE CASA em 21/set/2026, a52 P2: vive no js/app.js, na casca. Este arquivo passou a chegar
// só quando a Manutenção abre, e 11 módulos perguntam por ele para decidir entre a tela e o aviso de plano.
// As duas formas quebram sem ele: 9 chamam `!planoEhPro()` cru — ReferenceError dentro do render, a tela não
// desenha — e 2 perguntam com `typeof`, e aí a falta vira resposta errada calada: no js/parametrico.js a
// tela PRO abriria para quem é gratuito.)
