...
function contaCorpoTecnico() {
  const pane = contaPaneEl(); if (!pane) return;
  const ehCT = (typeof CONT !== "undefined" && CONT.ehCorpoTecnico && CONT.ehCorpoTecnico());
...
async function contaCTPainel(pane) {
  ...
  if (ehEditor) { try { fila = await CONT.fila(); } catch (e) {} }
...
async function contaSeguranca() {
  ...
  pane.innerHTML = `
    <div class="card conta-card">
      ...
      <div class="conta-grid2">
        <div class="field"><label>Nova senha</label><input type="password" id="ct-pw1" autocomplete="new-password" placeholder="Mínimo 8 caracteres"></div>
  ...
  pane.querySelector("#ct-pw-save").onclick = async function () {
    ...
    // Mínimo, teto de 72 bytes e confirmação: a mesma regra do cadastro (AUTH.validarSenhaNova, js/auth.js).
