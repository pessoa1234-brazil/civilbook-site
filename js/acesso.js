...
(function () {
  ...
  // = Supabase Auth → Email → Password requirements ("Lowercase, uppercase letters, digits and symbols", visto em
  // 18/set/2026): um caractere de cada conjunto. Cópia EXATA do AUTH.SENHA_TIPOS (js/auth.js, onde está a fonte no
  // código do supabase/auth e a lista do que muda junto); tests/acesso.check.mjs compara os dois com o painel. Mudou no
  // painel, muda aqui. Só é lido no salvarSenha (o check conta os usos: ninguém o altera depois de declarado).
  var SENHA_TIPOS = [
  ...
  // Os tipos que FALTAM na senha nova, conferidos como o servidor confere — corpo IDÊNTICO ao de AUTH._faltaTipos
  // (js/auth.js); tests/acesso.check.mjs compara os dois e roda. Antes, a senha sem os tipos ia ao servidor e voltava
  // recusada: no convite, era a primeira coisa que o convidado via.
  function faltaTipos(senha, tipos) {
  ...
  async function salvarSenha(ev) {
    ...
    var faltam = faltaTipos(a, SENHA_TIPOS);
    if (faltam) { erroSenha(faltam); return; }
